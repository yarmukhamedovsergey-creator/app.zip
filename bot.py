"""
USERNAME HUNTER v26.0 — FARM CACHE + WORD SESSION + FAST DB SEARCH
"""
...
DEFAULT_CONFIG = {
    ...
    "mode_anyword": False, "mode_mat": False, "mode_telegram": False,
    ...
    "farm_enabled": True,
    "farm_target_default": 60,
    "farm_target_beautiful": 60,
    "farm_target_meaningful": 60,
    "farm_fragment_ttl_min": 15,
}
...
def log_action(uid, action, details=""):
    ...
    try:
        c.execute(
            "INSERT INTO action_log (uid,action,details,created) VALUES (?,?,?,?)",
...
def get_action_log(limit=50):
    ...
    try:
        c.execute(
            "SELECT uid,action,details,created FROM action_log ORDER BY id DESC LIMIT ?",
            (limit,)
        )
        rows = c.fetchall()
    except:
...
_DIGRAPHS = [
    "bl","br","ch","cl","cr","dr","fl","fr","gl","gr","kn","pl","pr",
    "sc","sh","sk","sl","sm","sn","sp","st","sw","th","tr","tw","wr","zh"
]
_ENDINGS = [
    "ax","ex","ix","ox","en","on","an","er","or","ar","in","yn","us",
...
def gen_beautiful():
    ...
    if s == 1:
        ...
        for ch in pat:
            if ch == "c":
                p = [x for x in _C if x not in uc]
                l = random.choice(p if p else list(_C))
                w.append(l)
                uc.add(l)
            else:
                w.append(random.choice(_V))
        return "".join(w)
    elif s == 3:
        start = random.choice(_C) + random.choice(_V) + random.choice(_C)
        ...
        s1 = random.choice([
            "ka","ki","ko","ku","ra","ri","ro","ru","na","ni","no","nu",
            "ma","mi","mo","mu","ta","ti","to","la","li","lo","sa","si"
        ])
        s2 = random.choice([
            "zen","rix","lex","vin","ren","lin","rex","nox","vex","lux",
...
def gen_meaningful():
    pre = [
        "my", "go", "hi", "ok", "no", "up", "on", "in", "mr", "dj", "pro", "top", "hot", "big",
        "old", "new", "red", "max", "neo", "zen", "ice", "sun", "sky", "air", "sea", "own",
        "try", "run", "fly", "win", "get", "set", "fix", "mix", "pop", "raw", "now", "day",
        "one", "bit", "arc", "vox", "lux", "dot", "net", "web", "app", "sys", "bio", "geo",
        "eco", "cry", "pay", "buy", "sell", "low", "high", "fast", "slow", "hard", "soft"
    ]
    mid = [
        "cool", "fast", "best", "good", "real", "true", "dark", "wild", "bold", "epic", "mega",
        "gold", "blue", "easy", "mini", "deep", "kind", "wise", "calm", "warm", "vibe", "flow",
        "pure", "rare", "hard", "soft", "neon", "acid", "salt", "fire", "cold", "holy", "lost",
        "zero", "void", "null", "core", "main", "base", "peak", "smart", "dumb", "full", "empty"
    ]
    suf = [
        "bot", "dev", "pro", "man", "boy", "cat", "dog", "fox", "owl", "god", "war", "run",
        "fly", "win", "fan", "art", "lab", "hub", "app", "web", "net", "box", "job", "pay",
        "buy", "car", "map", "log", "key", "pin", "tag", "tip", "spy", "doc", "gem", "ink",
        "code", "core", "node", "link", "base", "site", "page", "room", "cell", "area", "zone",
        "point", "spot", "path", "way", "mode", "form", "type", "kind", "part", "unit", "item"
    ]
    ...
    if s == "ps":
        ...
        r = random.choice(suf) + random.choice(mid)
    elif s == "pn":
        r = random.choice(pre) + str(random.randint(1,99))
    else:
        r = random.choice(pre) + random.choice(pre) + random.choice(suf)
...
def gen_mat():
    roots = [
        "blyad","suka","nahui","pizdec","huy","pizda","ebat","mraz","gandon","shlyuha",
        "pidor","mudak","chmo","loh","xyilo","padla","ueban","tvar","blya","fck","wtf",
        "dmn","btch","azz","dick","fuk","shyt","pzdc","nah","suk","eblan","pidor","lox",
        "zalupa","dermo","gavno","zhopa","bzdun","obsos","gnida","ublud","padonok","ebun"
    ]
    pre = [
        "mr_","el_","big_","top_","my_","x_","da_","not_","im_","ya_","the_","super_",
        "mega_","ultra_","king_","real_","true_","best_","cool_","dirty_","bad_","mad_"
    ]
    suf = ["_bot","_pro","_god","_king","_gg","_top","_go","_dev","_fan","_life","off","on"]
    s = random.randint(1, 6)
    if s == 1:
        r = random.choice(pre) + random.choice(roots)
    elif s == 2:
        r = random.choice(roots) + random.choice(suf)
    elif s == 3:
        r = random.choice(pre) + random.choice(roots) + random.choice(suf)
    elif s == 4:
        r = random.choice(roots) + random.choice(roots[:15])
    elif s == 5:
        r = random.choice(roots) + str(random.randint(1,999))
    else:
        r = random.choice(pre) + random.choice(roots) + str(random.randint(0,99))
    r = r.replace("__", "_").strip("_")
    if len(r) < 5:
        r += random.choice(suf).strip("_")
    if len(r) > 15:
        r = r[:15]
    if not re.match(r'^[a-zA-Z][a-zA-Z0-9_]*$', r):
        return gen_default()
...
def gen_telegram():
    tg_core = [
        "tg","telegram","telega","durov","pavel","ton","gram","channel","chat","group",
        "premium","stars","wallet","fragment","anon","nft","proxy","mtproto","hub",
        "botfather","support","mobile","android","ios"
    ]
    tg_features = [
        "bot","admin","mod","owner","reply","forward","pin","edit","delete","mute",
        "ban","kick","invite","link","username","bio","photo","status","call","live",
        "cloud","data","cache","session","dc1","dc2","dc3","dc4","dc5"
    ]
    tg_slang = [
        "tgk","tgram","telga","pasha","tgbot","tgadm","tgpro","tgking",
        "tgboss","chatik","kanal","tele","tga","toncoin","durovish"
    ]
    roles = [
        "admin","owner","god","king","boss","pro","master","dev","fan","lover",
        "lord","main","super","mega","top","best","real","elite","alpha","prime"
    ]
    pre = [
        "my_","the_","im_","mr_","not_","only_","real_","true_","best_","top_",
        "pro_","super_","mega_","ultra_","neo_","meta_","hyper_","dark_","cyber_"
    ]
    suf = [
        "_pro","_god","_king","_x","_gg","_1","_top","_go","_bot","_dev","_fan","_life",
        "_love","_man","_boy","er","ist","ik","ka","off","on","hub","zone","net","web"
    ]
    verbs = ["hack", "code", "mine", "farm", "trade", "swap", "flip", "hold"]
    num = ["69", "67", "1337", "42", "52", "1488", "228", "666"]
...
def gen_word_combinations(word):
    ...
    w3 = [
        ...
        "max","mix","fix","pop","raw","day","one","bit","dot","top","hot","big",
        "old","new","red","ice","zen","sun","sky","air","sea","try","set","get"
    ]
    w4 = [
        "cook","shop","club","gang","crew","team","band","camp","city","town",
        "land","park","road","path","lake","moon","rain","wind","snow","gold",
        "coin","cash","bank","card","gift","show","play","game","song","film",
        "work","task","plan","goal","idea","mind","soul","life","love","hope",
        "dark","deep","fast","slow","hard","soft","wild","bold","wise","kind",
        "pure","true","real","good","best","mega","boss","lord","star","fire",
        "void","core","code","node","link","base","site","zone","mode","data",
        "tech","drip","trap","vibe","flow","grim","riot","punk","bass"
    ]
    w5 = [
        "craft","world","storm","flame","frost","steel","blade","night","light",
        "dream","ghost","power","force","magic","chaos","order","peace","death",
        "heart","brain","pulse","flash","spark","blaze","smoke","steam","stone",
        "earth","ocean","river","cloud","shade","ninja","titan","demon","beast",
        "money","trade","store","tower","house","guard","watch","point","score",
        "level","arena","rider","racer","pilot","chief","cyber","pixel","sonic",
        "turbo","ultra","super","hyper","alpha","omega","delta","sigma","prime",
        "elite","royal","mafia","cartel"
    ]

    pre_short = ["my","mr","el","la","de","go","no","hi","ok","up","on","in","dj","dr","mc"]
    pre_long = ["big","top","hot","old","new","red","ice","pro","neo","zen","raw","the","not","sir","max"]
    ...
    for u in results:
        ...
        if (
            ul not in seen
            and re.match(r'^[a-zA-Z][a-zA-Z0-9_]*$', ul)
            and 5 <= len(ul) <= 15
            and "__" not in ul
            and not ul.startswith("_")
            and not ul.endswith("_")
        ):
            valid.append(ul)
            seen.add(ul)
...
FARM_USER_MODES = ("default", "beautiful", "meaningful")


INVALID_WORDS = [
    "admin","support","help","test","telegram","bot","official",
...
def evaluate_username(u):
    ...
    if ln <= 3:
        score += 95; factors.append("🔥 Ультракороткий")
    elif ln <= 4:
        score += 80; factors.append("💎 Очень короткий")
    elif ln <= 5:
        score += 60; factors.append("✨ Короткий")
    elif ln <= 6:
        score += 30; factors.append("📏 Средний")
    else:
        score += 10; factors.append("📦 Длинный")

    if len(set(ul)) == 1 and ln >= 3:
        score += 90; factors.append("🔥 Моно")
    if ul == ul[::-1] and ln >= 3:
        score += 40; factors.append("🪞 Палиндром")
    if "_" in u:
        score += 5; factors.append("🔗 Разделитель")
    if ul.isalpha():
        score += 15; factors.append("🔤 Чистые буквы")
    if not any(c.isdigit() for c in ul):
        score += 10; factors.append("🅰️ Без цифр")

    vc = sum(1 for c in ul if c in _V)
    if 0.3 <= vc / max(len(ul), 1) <= 0.6:
        score += 15; factors.append("🗣 Произносимый")
    ...
    if score >= 150:
        ...
        pr, ra = "$100-$500", "💎 ХОРОШИЙ"
    elif score >= 40:
        pr, ra = "$20-$100", "✨ СТАНДАРТ"
    else:
        pr, ra = "$1-$20", "📦 ОБЫЧНЫЙ"
...
class AccountPool:
    def __init__(self):
        ...
        self.BASE_DELAY = 0.8
        self.MAX_DELAY = 8.0
        self.BUDGET_PER_MIN = 16
        self.MAX_ERROR_STREAK = 4
        self.FLOOD_REST_TIME = 420
        self.WARMUP_EXTRA_DELAY = 2.0
    ...
    async def init(self, accounts):
        ...
        if self.clients:
            self.word_reserved_idx = len(self.clients) - 1
            ...
            try:
                ...
                for i in range(len(self.clients)):
                    ...
                    if st == 'dead':
                        ...
                        self.status[i] = 'warming'
                        self.error_streak[i] = 0
                        self.adaptive_delay[i] = self.BASE_DELAY + 1
                ...
                if 0 < alive <= 2:
                    for i in range(len(self.clients)):
                        if self.status.get(i) in ('healthy', 'warming'):
                            self.adaptive_delay[i] = max(self.adaptive_delay[i], self.BASE_DELAY * 1.6)
    ...
    def _best(self, uid=None, allowed=None, excluded=None):
        now = time.time()
        cands = []
        ...
        for i in range(len(self.clients)):
            ...
            alive = sum(
                1 for j in range(len(self.clients))
                if self.status.get(j) in ('healthy', 'warming')
            )
            if alive <= 2:
                b = max(5, b // 2)

            if self.req_count.get(i, 0) >= b:
                continue

            d = self.adaptive_delay.get(i, self.BASE_DELAY)
            if st == 'warming':
                d += self.WARMUP_EXTRA_DELAY

            if now - self.last_used.get(i, 0) < d:
                continue
    ...
    async def _acquire(self, uid=None, timeout=45, allowed=None, excluded=None):
        dl = time.time() + timeout
        a = 0
        while time.time() < dl:
            async with self.lock:
                i = self._best(uid, allowed=allowed, excluded=excluded)
                if i is not None:
                    self.last_used[i] = time.time()
                    self.req_count[i] = self.req_count.get(i, 0) + 1
                    self.total_checks += 1
                    return i, self.clients[i]
            await asyncio.sleep(min(0.2 * (1.18 ** a), 1.5) + random.uniform(0, 0.15))
            a += 1
        return None, None

    def _ok(self, i):
        self.error_streak[i] = 0
        if self.status.get(i) == 'warming' and self.req_count.get(i, 0) >= 3:
            self.status[i] = 'healthy'
    ...
    def _err(self, i, flood=False, secs=0):
        ...
        if flood:
            self.cooldown_until[i] = time.time() + secs + random.randint(15, 35)
            ...
            self.adaptive_delay[i] = min(self.adaptive_delay.get(i, self.BASE_DELAY) * 1.4, self.MAX_DELAY)
            self.cooldown_until[i] = time.time() + 2
            self.status[i] = 'cooldown'

    async def _resolve(self, u, uid=None, allowed=None, excluded=None):
        i, c = await self._acquire(uid, 20, allowed=allowed, excluded=excluded)
        if c is None:
            return "no_session", -1
        try:
            await c(ResolveUsernameRequest(u))
            self._ok(i)
            return "taken", i
            ...
            self._err(i, True, e.seconds)
            return "flood", i
        except:
            self._err(i)
            return "error", i

    async def _check_avail(self, u, uid=None, allowed=None, excluded=None):
        i, c = await self._acquire(uid, 15, allowed=allowed, excluded=excluded)
        if c is None:
            return "no_session", -1
        try:
            ok = await c(AccountCheckUsername(u))
            self._ok(i)
            return ("free" if ok else "taken"), i
        except FloodWaitError as e:
            self._err(i, True, e.seconds)
            return "flood", i
        except:
    ...
    async def _botapi(self, u):
        try:
            ...
            msg = str(e).lower()
            if "not found" in msg or "chat not found" in msg:
                return "not_found"
            return "taken"
        except:
            return "error"
    ...
    async def strong_check(self, u, uid=None):
        if self.has_sessions():
            r1, _ = await self._check_avail(u, uid)
            if r1 == "taken":
                return "taken"
            if r1 != "free":
                return "skip"
            ...
            r2, _ = await self._resolve(u, uid)
            if r2 == "free":
                return "free"
    ...
    def remove_user(self, uid):
        for i in self.active_users:
            self.active_users[i].discard(uid)
    ...
    def detailed_status(self):
        ...
        for i in range(len(self.clients)):
            ...
            lines.append(
                f"{em}#{i+1} {st} d={self.adaptive_delay.get(i,0):.1f} "
                f"e={self.error_streak.get(i,0)} r={self.req_count.get(i,0)}"
            )
        return "\n".join(lines)
...
def get_word_reserved_idx():
    total = len(pool.clients)
    if total <= 0:
        return None
    idx = getattr(pool, "word_reserved_idx", None)
    if idx is None or idx < 0 or idx >= total:
        idx = total - 1
        pool.word_reserved_idx = idx
    return idx


def get_farm_allowed_indices():
    total = len(pool.clients)
    if total <= 0:
        return []
    if total == 1:
        return [0]
    reserved = get_word_reserved_idx()
    return [i for i in range(total) if i != reserved]


def get_word_allowed_indices():
    total = len(pool.clients)
    if total <= 0:
        return []
    idx = get_word_reserved_idx()
    return [idx] if idx is not None else []


def farm_live_session_count():
    inds = get_farm_allowed_indices()
    return sum(1 for i in inds if pool.status.get(i) in ("healthy", "warming"))


def word_live_session_count():
    inds = get_word_allowed_indices()
    return sum(1 for i in inds if pool.status.get(i) in ("healthy", "warming"))


async def farm_session_confirm_free(username):
    allowed = get_farm_allowed_indices()
    if not allowed or farm_live_session_count() <= 0:
        return "skip"

    r1, _ = await pool._check_avail(username, allowed=allowed)
    if r1 == "taken":
        return "taken"
    if r1 != "free":
        return "skip"
    ...
    r2, _ = await pool._resolve(username, allowed=allowed)
    if r2 == "free":
        return "free"
    if r2 in ("taken", "invalid"):
        return "taken"
...
async def word_session_confirm_free(username):
    allowed = get_word_allowed_indices()
    if not allowed or word_live_session_count() <= 0:
        return "skip"
...
def farm_get_targets():
    cfg = load_bot_config()
    return {
        "default": cfg.get("farm_target_default", 60),
        "beautiful": cfg.get("farm_target_beautiful", 60),
        "meaningful": cfg.get("farm_target_meaningful", 60),
    }


def farm_is_enabled():
    return load_bot_config().get("farm_enabled", True)


def farm_fragment_ttl_minutes():
    return load_bot_config().get("farm_fragment_ttl_min", 15)


def farm_known_username(username):
    conn = sqlite3.connect(DB)
    c = conn.cursor()
    c.execute("SELECT 1 FROM farm_cache WHERE username=? LIMIT 1", (username.lower(),))
    row = c.fetchone()
    conn.close()
    return row is not None


def farm_add_username(username, mode, fragment_status="unavailable", score=0):
    conn = sqlite3.connect(DB)
    c = conn.cursor()
    ok = False
    try:
        c.execute("""INSERT OR IGNORE INTO farm_cache
                     (username,mode,fragment_status,score,found_at,fragment_checked_at,issued,dead)
                     VALUES (?,?,?,?,?,?,0,0)""",
                  (username.lower(), mode, fragment_status, score, farm_now(), farm_now()))
        ok = c.rowcount > 0
    except:
        ok = False
    conn.commit()
...
def farm_available_counts():
    conn = sqlite3.connect(DB)
    c = conn.cursor()
    now_s = farm_now()
    counts = {m: 0 for m in FARM_USER_MODES}

    c.execute("""SELECT mode, COUNT(*) FROM farm_cache
                 WHERE issued=0 AND dead=0 AND (reserved_until='' OR reserved_until<?)
                 GROUP BY mode""", (now_s,))
...
def farm_pick_mode():
    counts = farm_available_counts()
    targets = farm_get_targets()

    deficits = []
    for mode in FARM_USER_MODES:
        cur = counts.get(mode, 0)
        target = targets.get(mode, 60)
        need = target - cur
        if need > 0:
            deficits.append((mode, need))

    if not deficits:
        return None

    deficits.sort(key=lambda x: x[1], reverse=True)
    top_need = deficits[0][1]
    top_modes = [m for m, need in deficits if need == top_need]
    return random.choice(top_modes)


def farm_cleanup_cache():
    conn = sqlite3.connect(DB)
    c = conn.cursor()

    old_dead = (datetime.now() - timedelta(hours=6)).strftime("%Y-%m-%d %H:%M:%S")
    old_issued = (datetime.now() - timedelta(hours=12)).strftime("%Y-%m-%d %H:%M:%S")
    old_active = (datetime.now() - timedelta(minutes=90)).strftime("%Y-%m-%d %H:%M:%S")

    try:
        c.execute("DELETE FROM farm_cache WHERE dead=1 AND found_at<?", (old_dead,))
        c.execute("DELETE FROM farm_cache WHERE issued=1 AND issued_at!='' AND issued_at<?", (old_issued,))
        c.execute("UPDATE farm_cache SET dead=1, reserved_until='' WHERE issued=0 AND dead=0 AND found_at<?", (old_active,))
    except:
        pass
...
def farm_stats():
    ...
    total_active = c.execute("""SELECT COUNT(*) FROM farm_cache
                                WHERE issued=0 AND dead=0 AND (reserved_until='' OR reserved_until<?)""",
                             (now_s,)).fetchone()[0]
    total_issued = c.execute("SELECT COUNT(*) FROM farm_cache WHERE issued=1").fetchone()[0]
    total_dead = c.execute("SELECT COUNT(*) FROM farm_cache WHERE dead=1").fetchone()[0]
    ...
    return {
        ...
        "dead_total": total_dead,
        "counts": counts,
        "farm_sessions": farm_live_session_count(),
        "word_sessions": word_live_session_count(),
        "word_reserved_idx": get_word_reserved_idx(),
    }


def farm_get_candidates(mode, limit=30):
    conn = sqlite3.connect(DB)
    conn.row_factory = sqlite3.Row
    c = conn.cursor()
    now_s = farm_now()

    c.execute("""SELECT * FROM farm_cache
                 WHERE mode=? AND issued=0 AND dead=0
                 AND (reserved_until='' OR reserved_until<?)
                 ORDER BY score DESC, id DESC
                 LIMIT ?""", (mode, now_s, limit))
    rows = [dict(r) for r in c.fetchall()]
    conn.close()
    return rows


def farm_get_candidates_any(limit=30, exclude_usernames=None):
    ...
    if exclude_usernames:
        ...
        q = f"""SELECT * FROM farm_cache
                WHERE issued=0 AND dead=0
                AND (reserved_until='' OR reserved_until<?)
                AND username NOT IN ({placeholders})
                ORDER BY score DESC, id DESC
                LIMIT ?"""
        params = [now_s] + exclude_usernames + [limit]
        c.execute(q, params)
    else:
        c.execute("""SELECT * FROM farm_cache
                     WHERE issued=0 AND dead=0
...
def farm_reserve_ids(ids, seconds=25):
    if not ids:
        return
    conn = sqlite3.connect(DB)
    c = conn.cursor()
    until = (datetime.now() + timedelta(seconds=seconds)).strftime("%Y-%m-%d %H:%M:%S")
    placeholders = ",".join("?" * len(ids))
    c.execute(f"UPDATE farm_cache SET reserved_until=? WHERE id IN ({placeholders})", [until] + ids)
    conn.commit()
    conn.close()


def farm_release_ids(ids):
    if not ids:
        return
    conn = sqlite3.connect(DB)
    c = conn.cursor()
    placeholders = ",".join("?" * len(ids))
    c.execute(f"UPDATE farm_cache SET reserved_until='' WHERE id IN ({placeholders})", ids)
    conn.commit()
    conn.close()


def farm_mark_issued_ids(ids):
    if not ids:
        return
    conn = sqlite3.connect(DB)
    c = conn.cursor()
    now_s = farm_now()
    placeholders = ",".join("?" * len(ids))
    c.execute(f"""UPDATE farm_cache
                  SET issued=1, issued_at=?, reserved_until=''
                  WHERE id IN ({placeholders})""", [now_s] + ids)
    conn.commit()
    conn.close()


def farm_mark_dead_ids(ids):
    if not ids:
        return
    conn = sqlite3.connect(DB)
    c = conn.cursor()
    placeholders = ",".join("?" * len(ids))
    c.execute(f"""UPDATE farm_cache
                  SET dead=1, reserved_until=''
                  WHERE id IN ({placeholders})""", ids)
    conn.commit()
    conn.close()


def farm_update_fragment(item_id, fragment_status):
    conn = sqlite3.connect(DB)
    c = conn.cursor()
    c.execute("""UPDATE farm_cache
                 SET fragment_status=?, fragment_checked_at=?
                 WHERE id=?""", (fragment_status, farm_now(), item_id))
    conn.commit()
    conn.close()


def farm_fragment_is_stale(fragment_checked_at):
    if not fragment_checked_at:
        return True
    try:
        dt = datetime.strptime(fragment_checked_at, "%Y-%m-%d %H:%M:%S")
        return (datetime.now() - dt).total_seconds() > farm_fragment_ttl_minutes() * 60
    except:
        return True


async def farm_hunt_one(mode):
    gen_func = SEARCH_MODES[mode]["func"]
    candidate = None
    ...
    tg = await farm_session_confirm_free(candidate)
    if tg != "free":
        return False

    fr = await check_fragment(candidate)
    if fr == "fragment":
        return False

    score = evaluate_username(candidate)["score"]
    return farm_add_username(candidate, mode, fr, score)


async def farm_worker(worker_id):
    await asyncio.sleep(10 + worker_id)

    while True:
        try:
            if not farm_is_enabled():
                await asyncio.sleep(5)
                continue

            if farm_live_session_count() <= 0:
                await asyncio.sleep(5)
                continue
            ...
            added = await farm_hunt_one(mode)

            if added:
                await asyncio.sleep(random.uniform(0.18, 0.32))
            else:
                await asyncio.sleep(random.uniform(0.35, 0.60))

        except asyncio.CancelledError:
            break
        except Exception as e:
            logger.error(f"Farm worker #{worker_id}: {e}")
            await asyncio.sleep(2)


async def do_issue_from_farm(mode, count, msg, uid):
    start = time.time()
    found = []
    attempts = 0

    pool1 = farm_get_candidates(mode, limit=max(20, count * 10))
    usernames1 = [x["username"] for x in pool1]

    if len(pool1) < count:
        extra = farm_get_candidates_any(limit=max(10, count * 8), exclude_usernames=usernames1)
        candidates = pool1 + extra
    else:
        candidates = pool1
    ...
    reserved_ids = [x["id"] for x in candidates]
    farm_reserve_ids(reserved_ids, seconds=25)
    ...
    try:
        for item in candidates:
            ...
            tg = await farm_session_confirm_free(u)
            if tg != "free":
                dead_ids.append(item["id"])
                continue

            frag = item.get("fragment_status", "unavailable")
            if farm_fragment_is_stale(item.get("fragment_checked_at", "")):
                frag = await check_fragment(u)
                farm_update_fragment(item["id"], frag)

            if frag == "fragment":
                dead_ids.append(item["id"])
                continue

            found.append({"username": u, "fragment": frag})
            issued_ids.append(item["id"])

        for item in candidates:
            if item["id"] not in issued_ids and item["id"] not in dead_ids and item["id"] not in release_ids:
                release_ids.append(item["id"])

        if dead_ids:
            farm_mark_dead_ids(dead_ids)
        if issued_ids:
            farm_mark_issued_ids(issued_ids)
        if release_ids:
            farm_release_ids(release_ids)

        return found, {"attempts": attempts, "elapsed": int(time.time() - start)}

    except:
        farm_release_ids(reserved_ids)
        raise
...
def init_db():
    ...
    c.execute("""CREATE TABLE IF NOT EXISTS farm_cache (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        username TEXT UNIQUE,
        mode TEXT DEFAULT '',
        fragment_status TEXT DEFAULT 'unavailable',
        score INTEGER DEFAULT 0,
        found_at TEXT DEFAULT '',
        fragment_checked_at TEXT DEFAULT '',
    ...
    for col, default in [
        ...
        try:
            c.execute(f"ALTER TABLE market ADD COLUMN {col} DEFAULT {default}")
        except:
            pass
...
def ensure_user(uid, uname=""):
    ...
    if not c.fetchone():
        c.execute(
            "INSERT INTO users (uid,uname,joined,free) VALUES (?,?,?,?)",
...
def get_user(uid):
    ...
    if not row:
        return {
            "uid": uid, "uname": "", "free": FREE_SEARCHES, "searches": 0, "sub_end": "",
...
def is_banned(uid):
    if rate_limiter.is_temp_banned(uid):
        return True
...
def get_balance(uid):
    return get_user(uid).get("balance", 0.0)
...
def process_referral(new_uid, ref_uid):
    ...
    c.execute(
        "INSERT INTO referrals (referrer_uid,referred_uid,referred_uname,created) VALUES (?,?,?,?)",
...
def get_pending_ref(uid):
    return get_user(uid).get("pending_ref", 0)
...
def can_roulette(uid):
    u = get_user(uid)
    lr = u.get("last_roulette","")
    if not lr:
        return True
    try:
        return (datetime.now() - datetime.strptime(lr,"%Y-%m-%d %H:%M")).days >= 7
    except:
        return True
...
def get_active_promotions():
    ...
    conn.close()
    return [{
        "id": r[0], "name": r[1], "ptype": r[2], "data": json.loads(r[3] or "{}"), "created": r[4],
...
def market_get_active_lots(limit=20, nft_only=False):
    ...
    conn.close()
    return [{
        "id":r[0],"seller":r[1],"type":r[2],"title":r[3],"desc":r[4],"price":r[5],
        "created":r[6],"promoted":r[7],"is_nft":r[8],"fragment_url":r[9]
    } for r in rows]
...
def market_resolve_dispute(lot_id, winner, admin_uid):
    ...
    if winner == "buyer":
        ...
        row = c.fetchone()
        charge_id = row[0] if row and row[0] else None
        conn.commit()
        conn.close()
...
def lootbox_open(uid):
    ...
    if roll <= 5:
        prize_type = "premium"; days = random.choice([3, 7]); give_subscription(uid, days); prize = f"💎 Premium {days} дней!"
    elif roll <= 15:
        prize_type = "vip"; days = random.choice([1, 3]); give_vip(uid, days); prize = f"🌟 VIP {days} дней!"
    elif roll <= 35:
        prize_type = "stars"; amount = random.choice([5, 10, 15, 20, 25, 50]); add_balance(uid, amount); prize = f"⭐ {amount} звёзд!"
    elif roll <= 60:
        prize_type = "searches"; count = random.choice([2, 3, 5, 7, 10]); add_extra_searches(uid, count); prize = f"🔍 {count} поисков!"
    elif roll <= 80:
        prize_type = "slot"; market_add_slot(uid); prize = f"📦 +1 слот маркета!"
    else:
        prize_type = "emoji"; prize = random.choice(["🧸 Плюшевый мишка!", "🃏 Джокер!", "🎭 Маска!", "🎉 Ура!"])
...
def check_promocode(code, uid, purchase_amount=0, purchase_type="all"):
    ...
    if not row:
        conn.close()
        return {"valid":False,"reason":"Не найден"}
    ...
    if promo.get("expires",""):
        try:
            if datetime.strptime(promo["expires"],"%Y-%m-%d %H:%M") < datetime.now():
                conn.close()
                return {"valid":False,"reason":"Истёк"}
        except:
            pass
    if promo["used_count"] >= promo["max_uses"]:
        conn.close()
        return {"valid":False,"reason":"Исчерпан"}
    c.execute("SELECT COUNT(*) FROM promocode_uses WHERE code=? AND uid=?", (code.upper(), uid))
    if c.fetchone()[0] > 0:
        conn.close()
        return {"valid":False,"reason":"Уже использован"}
    if promo["min_purchase"] > 0 and purchase_amount < promo["min_purchase"]:
        conn.close()
        return {"valid":False,"reason":f"Мин. покупка {promo['min_purchase']}⭐"}
...
def exchange_accept(eid, partner_uid, partner_offer):
    ...
    c = conn.cursor()
    deadline = (datetime.now() + timedelta(hours=MARKET_ESCROW_HOURS)).strftime("%Y-%m-%d %H:%M")
    c.execute("""UPDATE exchanges SET 
                 partner_uid=?, partner_offer=?,
                 status='escrow', escrow_deadline=?
...
def exchange_confirm(eid, uid):
    conn = sqlite3.connect(DB)
    c = conn.cursor()
    c.execute("""SELECT initiator_uid, partner_uid, status,
    ...
    c.execute("""SELECT initiator_confirmed, partner_confirmed,
...
async def check_fragment_nft(username):
    try:
        async with http_session.get(
            ...
            if resp.status != 200:
                return {"is_nft": False, "status": "unknown"}
            text = await resp.text()
            if any(x in text for x in ["Sold", "sold", "Assigned", "assigned", "Owner", "owner", "Status:&nbsp;Taken", "tm-value-sold"]):
...
async def show_user_panel(msg_or_cb, target_uid):
    ...
    if is_ban:
        kb.button(text="✅ Разбан", callback_data=f"au_unban_{target_uid}")
    else:
        kb.button(text="🚫 Бан", callback_data=f"au_ban_{target_uid}")
...
async def cmd_start(msg: Message, command: CommandObject):
    ...
    if is_banned(uid):
        ...
        if rem > 0:
            await msg.answer(f"🚫 Блокировка. Подождите {rem} мин.")
        else:
            await msg.answer("🚫 Заблокированы.")
        return
    ...
    if command.args and command.args.startswith("ref_") and is_new:
        try:
            ...
            if ref_id != uid:
                ...
                await msg.answer("🤖 <b>Подтвердите:</b>", reply_markup=kb.as_markup(), parse_mode="HTML")
                return
        except:
            pass
...
async def cb_captcha(cb: CallbackQuery):
    ...
    if ref_uid and ref_uid != uid:
        ...
        set_pending_ref(uid, 0)
        set_captcha_passed(uid)
        if ok:
            log_action(uid, "referral_join", str(ref_uid))
            try:
                await bot.send_message(ref_uid, f"🎉 Новый реферал! <b>+{REF_BONUS}</b>", parse_mode="HTML")
            except:
                pass
    else:
        set_captcha_passed(uid)
...
async def cb_search(cb: CallbackQuery):
    ...
    text = (
        f"🔍 <b>Поиск</b>\n\n"
        f"📦 Обычные режимы — <b>готовые юзы из базы</b>\n"
        f"🎯 По слову — <b>живая проверка</b> на отдельной сессии\n\n"
    )

    kb.button(text=f"🎲 Дефолт ({counts.get('default', 0)})", callback_data="farmgo_default")
    text += f"✅ <b>🎲 Дефолт</b> — в базе <code>{counts.get('default', 0)}</code>\n"

    if is_prem:
        kb.button(text=f"💎 Красивые ({counts.get('beautiful', 0)})", callback_data="farmgo_beautiful")
        text += f"✅ <b>💎 Красивые</b> — в базе <code>{counts.get('beautiful', 0)}</code>\n"
    else:
        kb.button(text=f"🔒 💎 Красивые ({counts.get('beautiful', 0)})", callback_data="need_prem")
        text += "🔒 <b>💎 Красивые</b> — нужен Premium\n"

    if is_prem:
        kb.button(text=f"📖 Словарные ({counts.get('meaningful', 0)})", callback_data="farmgo_meaningful")
        text += f"✅ <b>📖 Словарные</b> — в базе <code>{counts.get('meaningful', 0)}</code>\n"
    else:
        kb.button(text=f"🔒 📖 Словарные ({counts.get('meaningful', 0)})", callback_data="need_prem")
        text += "🔒 <b>📖 Словарные</b> — нужен Premium\n"

    if is_prem:
        kb.button(text="🎯 По слову", callback_data="cmd_thematic")
        text += "✅ <b>🎯 По слову</b> — отдельная живая сессия\n"
    else:
        kb.button(text="🔒 🎯 По слову", callback_data="need_prem")
        text += "🔒 <b>🎯 По слову</b> — нужен Premium\n"
...
@dp.callback_query(F.data.startswith("farmgo_"))
async def cb_farmgo(cb: CallbackQuery):
    uid = cb.from_user.id
    await answer_cb(cb)
    ...
    mode = cb.data[7:]
    if mode not in FARM_USER_MODES:
        return
    ...
    if not can_search(uid):
        kb = InlineKeyboardBuilder()
        kb.button(text="🏪 Магазин", callback_data="cmd_shop")
        kb.button(text="🔙", callback_data="cmd_search")
        kb.adjust(1)
        await edit_msg(cb.message, "⛔️ <b>Поиски закончились!</b>", kb.as_markup())
        return

    if uid in searching_users:
        await answer_cb(cb, "⏳ Уже идёт поиск!", show_alert=True)
        return

    searching_users.add(uid)
    try:
        available = farm_available_counts().get(mode, 0)
        await edit_msg(
            cb.message,
            f"📦 <b>{mi['emoji']} {mi['name']}</b>\n\n"
            f"Беру готовые юзы из базы...\n"
            f"📦 В базе: <code>{available}</code>"
        )

        use_search(uid)
        log_action(uid, "farm_issue", mode)

        count = get_search_count(uid)
        found, stats = await do_issue_from_farm(mode, count, cb.message, uid)

        kb = InlineKeyboardBuilder()
        text = format_results(found, stats, mi["name"])

        if can_search(uid):
            kb.button(text="🔄 Ещё", callback_data=f"farmgo_{mode}")
...
async def cb_thematic(cb: CallbackQuery):
    ...
    if not has_subscription(uid) and uid not in ADMIN_IDS:
        ...
        await edit_msg(
            cb.message,
            "🔒 <b>Нужен Premium!</b>\n\n"
            "Поиск по слову доступен с Premium подпиской.",
            kb.as_markup()
        )
        return
    ...
    reserved = get_word_reserved_idx()
    rs = f"#{reserved + 1}" if reserved is not None else "—"

    await edit_msg(
        cb.message,
        f"🎯 <b>Поиск по слову</b>\n\n"
        f"🔑 Выделенная сессия: <code>{rs}</code>\n\n"
        f"Введите слово:\n\n"
        f"• <code>crypto</code>\n"
        f"• <code>mef</code>\n"
        f"• <code>dark</code>\n"
...
async def cb_util_mass(cb: CallbackQuery):
    ...
    await edit_msg(cb.message,
        ...
        "Отправьте юзернеймы по одному на строку\n"
...
async def cb_del_pat(cb: CallbackQuery):
    await answer_cb(cb)
    user_states[cb.from_user.id] = {"action":"delete_pattern"}
    kb = InlineKeyboardBuilder()
    kb.button(text="❌", callback_data="cmd_utils")
    await edit_msg(cb.message,
...
async def cb_mon_new(cb: CallbackQuery):
    ...
    if get_monitor_count(uid) >= get_monitor_limit(uid):
        ...
        kb.button(text="🔙", callback_data="cmd_monitors")
        kb.adjust(1)
        await edit_msg(cb.message, "❌ Лимит слотов", kb.as_markup())
        return
...
async def cb_mon_add(cb: CallbackQuery):
    ...
    if un == "new":
        return
    if get_monitor_count(uid) >= get_monitor_limit(uid):
        await answer_cb(cb, "❌ Лимит", show_alert=True)
        return
    add_monitor(uid, un)
    log_action(uid, "monitor_add", un)
    kb = InlineKeyboardBuilder()
    kb.button(text="👁 Мои", callback_data="cmd_monitors")
    kb.button(text="🔙", callback_data="cmd_menu")
    kb.adjust(1)
...
async def cb_mon_del(cb: CallbackQuery):
    uid = cb.from_user.id
    await answer_cb(cb)
    remove_monitor(int(cb.data[8:]), uid)
...
async def cb_paybal_searches(cb: CallbackQuery):
    ...
    bal = get_balance(uid)
    if bal < cost:
        await answer_cb(cb, f"❌ Нужно {cost}⭐, у вас {bal:.1f}⭐", show_alert=True)
        return
...
async def cb_paybal_prem(cb: CallbackQuery):
    ...
    bal = get_balance(uid)
    if bal < p["stars"]:
        await answer_cb(cb, "❌ Недостаточно средств", show_alert=True)
        return
...
async def cb_buyvip(cb: CallbackQuery):
    ...
    if not vp:
        ...
        return
    if not has_subscription(uid) and uid not in ADMIN_IDS:
        await answer_cb(cb,"⚠️ Сначала купите Premium!",show_alert=True)
        return
    await answer_cb(cb)
    bal = get_balance(uid)
    kb = InlineKeyboardBuilder()
...
async def cb_market_nft(cb: CallbackQuery):
    ...
    for lot in lots:
        ...
        if lot.get("fragment_url"):
            text += f"   ?? <a href='{lot['fragment_url']}'>Fragment</a>\n"
        text += "\n"
        kb.button(text=f"?? {lot['title'][:20]} ({lot['price']}?)", callback_data=f"mlot_{lot['id']}")
    if not lots:
        text += "<i>Нет NFT лотов</i>"
...
async def cb_mlot(cb: CallbackQuery):
    ...
    if not lot or lot["status"] != "active":
        kb = InlineKeyboardBuilder()
        kb.button(text="??", callback_data="market_browse")
        await edit_msg(cb.message, "? Лот не найден", kb.as_markup())
        return
...
async def cb_mbuy(cb: CallbackQuery):
    ...
    lot = market_get_lot(lot_id)
    if not lot or lot["status"] != "active":
        await answer_cb(cb, "? Лот уже продан!", show_alert=True)
        return
    if lot["seller_uid"] == uid:
        await answer_cb(cb, "? Нельзя купить свой лот!", show_alert=True)
        return
...
async def cb_paybal_lot(cb: CallbackQuery):
    ...
    lot = market_get_lot(lot_id)
    if not lot or lot["status"] != "active":
        await answer_cb(cb, "? Лот продан!", show_alert=True)
        return
    if lot["seller_uid"] == uid:
        await answer_cb(cb, "? Свой лот!", show_alert=True)
        return

    price = int(lot["price"])
    bal = get_balance(uid)
    if bal < price:
        await answer_cb(cb, f"? Нужно {price}?, у вас {bal:.1f}?", show_alert=True)
        return
    ...
    if not ok:
        ...
        await answer_cb(cb, "? Ошибка покупки!", show_alert=True)
        return
    ...
    kb.button(text="?? Открыть спор", callback_data=f"mdispute_{lot_id}")
    kb.adjust(1)
    ...
    try:
        await bot.send_message(lot["seller_uid"],
...
async def cb_paystars_lot(cb: CallbackQuery):
    ...
    if lot["seller_uid"] == uid:
        await answer_cb(cb, "? Свой лот!", show_alert=True)
        return
    rub = int(lot["price"] * STAR_TO_RUB)
    await redirect_payment(uid, f"?? {lot['title'][:50]}", lot["price"], rub, f"mlot_{lot_id}")
...
async def cb_mpromo(cb: CallbackQuery):
    ...
    kb = InlineKeyboardBuilder()
    kb.button(text="?", callback_data=f"mlot_{lot_id}")
...
async def cb_mcancel(cb: CallbackQuery):
    ...
    market_cancel_lot(lot_id, uid)
    log_action(uid, "market_cancel", str(lot_id))
    await cb_market(cb)
...
async def cb_msellerok(cb: CallbackQuery):
    uid = cb.from_user.id
    await answer_cb(cb)
    lot_id = int(cb.data[10:])
    lot = market_get_lot(lot_id)
    if not lot or lot["seller_uid"] != uid:
        return
    ...
    if completed:
        ...
        try:
            await bot.send_message(lot["buyer_uid"], "? Сделка завершена!", parse_mode="HTML")
        except:
            pass
    else:
...
async def cb_mbuyerok(cb: CallbackQuery):
    ...
    lot_id = int(cb.data[9:])
    lot = market_get_lot(lot_id)
    if not lot or lot["buyer_uid"] != uid:
        return
    ...
    if completed:
        ...
        payout = int(lot["price"] * (1 - MARKET_COMMISSION))
        try:
            await bot.send_message(lot["seller_uid"], f"? Сделка завершена!\n?? +{payout}?", parse_mode="HTML")
        except:
            pass
    else:
...
async def cb_mdispute(cb: CallbackQuery):
    ...
    kb = InlineKeyboardBuilder()
    kb.button(text="?", callback_data="market_my_purchases")
    await edit_msg(cb.message, "?? <b>Причина спора:</b>", kb.as_markup())
...
async def cb_sell_nft(cb: CallbackQuery):
    ...
    kb = InlineKeyboardBuilder()
    kb.button(text="?", callback_data="market_nft")
    await edit_msg(cb.message,
...
async def cb_msell_type(cb: CallbackQuery):
    ...
    kb = InlineKeyboardBuilder()
    kb.button(text="?", callback_data="market_sell")
...
async def cb_mrev(cb: CallbackQuery):
    ...
    for r in reviews:
        fu = get_user(r['from'])
        fn = f"@{fu.get('uname','')}" if fu.get('uname') else f"ID:{r['from']}"
        text += f"{'?'*r['rating']} {fn}\n{r['text']}\n\n"
    if not reviews:
        text += "<i>Нет отзывов</i>"
    kb = InlineKeyboardBuilder()
    kb.button(text="??", callback_data="cmd_market")
    kb.adjust(1)
    await edit_msg(cb.message, text, kb.as_markup())


# ======================= КОЛЕСО / ЛУТБОКС / ОБМЕННИК =======================
...
async def cb_wheel_free(cb: CallbackQuery):
    ...
    if wheel_free_spins_today(uid) >= WHEEL_FREE_DAILY:
        await answer_cb(cb, "? Бесплатные кончились!", show_alert=True)
        return
    for e in ["??","??","??","??","?","??"]:
...
async def cb_lootbox_open(cb: CallbackQuery):
    ...
    if not lootbox_can_open(uid):
        await answer_cb(cb, "? Подождите!", show_alert=True)
        return
...
async def cb_exview(cb: CallbackQuery):
    ...
    if not ex or ex["status"] != "open":
        kb = InlineKeyboardBuilder()
        kb.button(text="??", callback_data="market_exchange")
        await edit_msg(cb.message, "? Обмен закрыт", kb.as_markup())
        return
...
async def cb_exconfirm(cb: CallbackQuery):
    ...
    if completed and ex_data:
        ...
        await edit_msg(cb.message,
            ...
            f"?? Не забудь — юзернейм нужно <b>успеть забрать</b> до того как его займут!")
        ...
        log_action(uid, "exchange_complete", f"#{eid} {init_offer}-{part_offer}")
    else:
        ex = exchange_get(eid)
        if not ex:
            await edit_msg(cb.message, "? Обмен не найден")
        ...
        await edit_msg(cb.message,
            ...
            f"?? Как только они нажмут подтвердить — вы оба получите юзернеймы автоматически",
            kb.as_markup())


# ======================= CALLBACKS: ПРОФИЛЬ / РУЛЕТКА / РЕФЫ / ДОНАТ =======================
...
async def cb_profile(cb: CallbackQuery):
    ...
    kb = InlineKeyboardBuilder()
    kb.button(text="?? Выкл авто" if ar_on else "?? Вкл авто", callback_data="toggle_renew")
    kb.button(text="?? История", callback_data="util_hist")
    ...
    if is_prem and is_button_enabled("roulette"):
...
async def cb_wd(cb: CallbackQuery):
    ...
    await edit_msg(cb.message,
        f"?? <b>Вывод</b>\n\n"
        f"?? {bal:.1f}? ({bal*STAR_TO_RUB:.0f}?)\n"
        f"Мин: {MIN_WITHDRAW}?\n\n"
        f"Сумма:", kb.as_markup())
...
async def cb_roulette(cb: CallbackQuery):
    ...
    if not can_roulette(uid):
        kb = InlineKeyboardBuilder()
        kb.button(text="??", callback_data="cmd_profile")
        await edit_msg(cb.message, "? Через неделю", kb.as_markup())
        return
    kb = InlineKeyboardBuilder()
...
async def cb_rspin(cb: CallbackQuery):
    ...
    roll = random.randint(1,100)
    if roll <= 40:
        days = random.choice([1,2]); give_subscription(uid,days); prize=f"?? +{days} дн!"
    elif roll <= 70:
        s = random.choice([1,2,3,5]); add_extra_searches(uid,s); prize=f"?? +{s} поисков!"
    elif roll <= 90:
        st = random.choice([5,10,15]); add_balance(uid,st); prize=f"? +{st} звёзд!"
    else:
        days = random.choice([3,5,7]); give_subscription(uid,days); prize=f"?? ДЖЕКПОТ! +{days} дн!"
    kb = InlineKeyboardBuilder()
    kb.button(text="??", callback_data="cmd_profile")
    kb.button(text="??", callback_data="cmd_menu")
    kb.adjust(1)
...
async def cb_ref(cb: CallbackQuery):
    ...
    await edit_msg(cb.message,
        f"?? <b>Рефералы</b>\n\n"
        f"?? <code>{u.get('ref_count',0)}</code>\n"
        f"?? <code>{u.get('balance',0):.1f}</code> ? (4%)\n"
...
async def cb_my_refs(cb: CallbackQuery):
    ...
    if not refs:
        text += "<i>Пусто</i>"
    kb = InlineKeyboardBuilder()
    kb.button(text="??", callback_data="cmd_referral")
    kb.adjust(1)
    await edit_msg(cb.message, text, kb.as_markup())
...
async def cb_pv(cb: CallbackQuery):
    ...
    if not promo:
        kb = InlineKeyboardBuilder()
        kb.button(text="??", callback_data="cmd_menu")
        await edit_msg(cb.message, "? Завершена", kb.as_markup())
        return

    if promo["ptype"] == "ref_contest":
        text = "?? <b>Реферальный конкурс!</b>\n\nПриглашайте друзей!\n\n?? Призы ТОП-5"
        kb = InlineKeyboardBuilder()
        kb.button(text="?? Топ", callback_data=f"pt_{pid}")
        kb.button(text="?? Ссылка", callback_data="cmd_referral")
        kb.button(text="??", callback_data="cmd_menu")
        kb.adjust(1)
    else:
...
async def cb_pt(cb: CallbackQuery):
    ...
    kb = InlineKeyboardBuilder()
    kb.button(text="?? Ссылка", callback_data="cmd_referral")
    kb.button(text="??", callback_data=f"pv_{pid}")
    kb.adjust(1)
...
async def cb_tt(cb: CallbackQuery):
    ...
    await edit_msg(cb.message,
        f"?? <b>TikTok</b>\n\n"
        f"1?? Найди видео\n"
        f"2?? {TIKTOK_SCREENSHOTS_NEEDED} комментов\n"
        f"?? <code>{TIKTOK_COMMENT_TEXT}</code>\n"
        f"3?? Скрины\n"
        f"4?? ?? {TIKTOK_REWARD_GIFT}",
        kb.as_markup())
...
async def cb_tg_tt(cb: CallbackQuery):
    ...
    if not tiktok_can_submit(uid):
        kb = InlineKeyboardBuilder()
        kb.button(text="??", callback_data="cmd_tiktok")
        await edit_msg(cb.message, "? Лимит на сегодня", kb.as_markup())
        return
    tid = task_create(uid)
    user_states[uid] = {"action":"tiktok_proof","task_id":tid,"photos":0}
    kb = InlineKeyboardBuilder()
    kb.button(text="?", callback_data="tt_cancel")
...
async def cb_ta(cb: CallbackQuery):
    ...
    if uid:
        ...
        try:
            await cb.message.edit_text(f"? #{tid} одобрено")
        except:
            pass
        try:
            await bot.send_message(uid, f"?? Одобрено! ?? {TIKTOK_REWARD_GIFT}")
        except:
            pass
...
async def cb_tr(cb: CallbackQuery):
    ...
    if uid:
        ...
        try:
            await cb.message.edit_text(f"? #{tid} отклонено")
        except:
            pass
        try:
            await bot.send_message(uid, "? Задание отклонено. Попробуйте снова.")
        except:
            pass
...
async def handle_photo(msg: Message):
    ...
    for aid in ADMIN_IDS:
        try:
            ...
            akb.button(text="?", callback_data=f"ta_{tid}")
            akb.button(text="?", callback_data=f"tr_{tid}")
            akb.adjust(2)
            ...
            for i in range(0, len(file_ids), 10):
                ...
                if i == 0:
                    media[0].caption = f"#{tid} | {display}"
                await bot.send_media_group(aid, media)
        except Exception as e:
            logger.error(f"TT {aid}: {e}")
    ...
    if parts[0] == "sub" and len(parts) >= 3:
        ...
        if p:
            ...
            if ref_uid and ref_uid != tuid:
                ...
                try:
                    await bot.send_message(ref_uid, f"?? +<code>{comm}</code>? реф. бонус", parse_mode="HTML")
                except:
                    pass
        ...
        if p:
            ...
            try:
                await bot.send_message(tuid, f"?? Вам подарили <b>{p['label']}</b>!\nДо: {end}", parse_mode="HTML")
            except:
                pass
        ...
        amount = int(parts[1])
        tuid = int(parts[2])
        add_balance(tuid, amount)
        log_action(tuid,"topup",str(amount))
        ...
        count = int(parts[1])
        tuid = int(parts[2])
        add_extra_searches(tuid, count)
        log_action(tuid,"buy_searches",str(count))
        await msg.answer(f"? <b>+{count} поисков!</b>", parse_mode="HTML")
        await notify_admins(f"?? <b>ПОКУПКА</b>\n?? {display}\n?? +{count} поисков\n? {count*SEARCH_PRICE_STARS}?")

    elif parts[0] == "donate" and len(parts) >= 3:
        amt = int(parts[1])
        log_action(uid,"donate",str(amt))
        await msg.answer("?? <b>Спасибо!</b>", parse_mode="HTML")
        await notify_admins(f"?? <b>ДОНАТ</b>\n?? {display}\n? {amt}?")
        ...
        if not lot or lot["status"] != "active":
            try:
                ...
                await msg.answer("? Лот продан! Напишите админу для возврата.")
            return
        ...
        if not ok:
            try:
                ...
                await msg.answer("? Ошибка! Напишите админу.")
            return

        conn = sqlite3.connect(DB)
        c = conn.cursor()
        c.execute("UPDATE market SET charge_id=? WHERE id=?", (msg.successful_payment.telegram_payment_charge_id, lot_id))
        conn.commit()
        conn.close()
        ...
        for e in ["??","??","??","??","?","??"]:
            try:
                await bot.send_message(uid, f"{e} Крутим...")
            except:
                break
            await asyncio.sleep(0.3)
...
async def cb_wdo(cb: CallbackQuery):
    ...
    if r:
        ...
        try:
            await bot.send_message(r["uid"], f"? #{wid} {r['amount']:.1f}? одобрен!")
        except:
            pass
...
async def cb_wdn(cb: CallbackQuery):
    ...
    if r:
        ...
        try:
            await bot.send_message(r["uid"], f"? #{wid} отклонён")
        except:
            pass
...
async def handle_text(msg: Message):
    ...
    if action == "activate":
        ...
        if r:
            log_action(uid,"key",msg.text.strip())
            await msg.answer(f"?? {r['days']}дн до {r['end']}", parse_mode="HTML")
        else:
    ...
    if action == "evaluate":
        ...
        if not validate_username(un):
            await msg.answer("? Мин 5 символов")
            return
        wm = await msg.answer("?...")
        tg = await check_username(un)
        fr = await check_fragment(un)
        tgs = {"free":"? Свободен","taken":"? Занят","error":"??"}.get(tg,"?")
    ...
    if action == "quick_check":
        ...
        wm = await msg.answer("?...")
        tg = await check_username(un)
        st = {"free":"? Свободен!","taken":"? Занят","error":"??"}.get(tg,"?")
        kb = InlineKeyboardBuilder()
        if tg == "free":
            kb.button(text="?? Мониторинг", callback_data=f"mon_add_{un}")
    ...
    if action == "thematic_search":
        ...
        if len(word) < 2 or len(word) > 20:
            await msg.answer("? Слово от 2 до 20 символов")
            return
        if not re.match(r'^[a-zA-Z0-9_]+$', word):
            await msg.answer("? Только латиница, цифры, _")
            return
        if not has_subscription(uid) and uid not in ADMIN_IDS:
            await msg.answer("?? Нужен Premium!")
            return
        if not can_search(uid):
            await msg.answer("?? Поиски закончились!")
            return
        if word_live_session_count() <= 0:
            await msg.answer("?? Выделенная сессия для поиска по слову сейчас недоступна.")
            return

        if uid not in ADMIN_IDS:
            if uid in searching_users:
                await msg.answer("? Уже идёт поиск!")
                return
            cd = user_search_cooldown.get(uid,0)
            rem = SEARCH_COOLDOWN-(time.time()-cd)
            if rem > 0:
                await msg.answer(f"? {int(rem)} сек.")
                return

        searching_users.add(uid)
        try:
            log_action(uid,"word_search",word)
            use_search(uid)

            wm = await msg.answer(
                f"?? <b>По слову: {word}</b>\n\n"
                f"?? Выделенная сессия: <code>#{(get_word_reserved_idx() or 0) + 1}</code>\n"
                f"? Ищу..."
            )
            ...
            for u in gen_word_combinations(word):
                if len(found) >= count or attempts >= 350:
                    break
                if u in checked:
                    continue
                checked.add(u)
                ...
                r = await word_session_confirm_free(u)
                if r == "free":
            ...
            stats = {"attempts": attempts, "elapsed": int(time.time() - start)}

            kb = InlineKeyboardBuilder()
            text = format_results(found, stats, f"По слову: {word}")
    ...
    if action == "msell_title":
        ...
        if len(title) < 3 or len(title) > 100:
            await msg.answer("? 3-100 символов")
            return
        user_states[uid] = {"action":"msell_desc","mtype":state["mtype"],"title":title}
        await msg.answer("?? <b>Описание:</b>", parse_mode="HTML")
        return
    ...
    if action == "msell_desc":
        ...
        await msg.answer(f"?? <b>Цена (?):</b>\n{MARKET_MIN_PRICE} до {MARKET_MAX_PRICE}", parse_mode="HTML")
        return

    if action == "msell_price":
        try:
            price = int(msg.text.strip())
            assert MARKET_MIN_PRICE <= price <= MARKET_MAX_PRICE
        except:
            await msg.answer(f"? {MARKET_MIN_PRICE}-{MARKET_MAX_PRICE}")
            return
        user_states.pop(uid,None)
        lot_id = market_create_lot(uid,state["mtype"],state["title"],state["desc"],price)
        log_action(uid,"market_create",str(lot_id))
        await redirect_payment(uid, f"?? Размещение лота #{lot_id}", MARKET_LISTING_FEE, None, "market_sell")
        return

    if action == "msell_nft_title":
        username = msg.text.strip().replace("@","").lower()
        if not validate_username(username):
            await msg.answer("? Неверный юзернейм")
            return
        ...
        if not frag["is_nft"]:
            try:
                ...
                pass
            await msg.answer("? <b>Этот юзернейм не NFT на Fragment!</b>\n\nМожно продавать только NFT юзернеймы купленные на Fragment.", parse_mode="HTML")
            return
        if frag["status"] == "on_sale":
            try:
                ...
                pass
            await msg.answer("?? <b>Этот юз ещё на продаже на Fragment!</b>\n\nСначала купите его, потом продавайте здесь.", parse_mode="HTML")
            return
    ...
    if action == "msell_nft_price":
        ...
        await redirect_payment(uid, f"?? Размещение NFT #{lot_id}", MARKET_NFT_LISTING_FEE, None, "market_nft")
        return
    ...
    if action == "market_enter_promo":
        ...
        if not result["valid"]:
            await msg.answer(f"? {result.get('reason','')}")
            return
        discount = result["discount"]
        final = max(1, lot["price"] - discount)
        ...
        await bot.send_invoice(uid,
            ...
            prices=[LabeledPrice(label=lot["title"][:50], amount=final)])
        return
    ...
    if action == "exchange_counter":
        ...
        confirm_kb.button(text="? Я готов отдать юз", callback_data=f"exconfirm_{eid}")
        confirm_kb.adjust(1)
    ...
    if action == "monitor_add":
        ...
        add_monitor(uid, un)
        log_action(uid,"monitor_add",un)
        await msg.answer(f"? @{un} на мониторинге")
        return

    if action == "delete_pattern":
        user_states.pop(uid,None)
        pattern = msg.text.strip().lower().replace("@","")
        if len(pattern) < 2:
        ...
        kb.adjust(1)
        await msg.answer(
            f"?? Удалено <code>{deleted}</code> записей по шаблону <code>{pattern}</code>",
            reply_markup=kb.as_markup(),
            parse_mode="HTML"
        )
        return
    ...
    if action == "withdraw_amount":
        ...
        try:
            ...
            await msg.answer(f"? Мин {MIN_WITHDRAW}?")
            return
        bal = get_balance(uid)
        if amount > bal:
            await msg.answer("? Мало")
            return
        ...
        for aid in ADMIN_IDS:
            try:
                ...
                akb.button(text="?", callback_data=f"wd_no_{wid}")
                akb.adjust(2)
    ...
    if action == "admin_broadcast_text":
        ...
        for i, tu in enumerate(aus):
            ...
            if (i+1) % 50 == 0:
                try:
                    await sm.edit_text(f"?? {i+1}/{len(aus)} ?{s} ?{f}")
                except:
                    pass
            await asyncio.sleep(0.05)
        log_action(uid,"broadcast",f"s={s} f={f}")
        try:
            await sm.edit_text(f"? ?{s} ?{f}")
        except:
            pass
        return
    ...
    if action == "admin_give_user":
        ...
        await msg.answer(f"?? Дней для <code>{target}</code>?", parse_mode="HTML")
        return
    ...
    if action == "admin_give_days":
        ...
        try:
            await bot.send_message(target, f"?? <b>{days}дн</b> до <b>{end}</b>!", parse_mode="HTML")
        except:
            pass
        return
    ...
    if action == "admin_key_days":
        ...
        log_action(uid,"key_gen",key)
        await msg.answer(f"?? <code>{key}</code> — {days}дн", parse_mode="HTML")
        return
    ...
    if action == "admin_unban_input":
        ...
        if not target:
            ...
            return
        unban_user(target)
        log_action(uid,"unban",str(target))
    ...
    if action == "admin_promo_type":
        user_states.pop(uid,None)
        pid = create_promotion(state.get("name"), msg.text.strip().lower(), state.get("btn",""))
        log_action(uid,"promo",str(pid))
        await msg.answer(f"? #{pid}")
        return
    ...
    if action == "admin_user_set_free":
        ...
        try:
            ...
            await msg.answer("?")
            return
        set_free_searches(state["target"], count)
        log_action(uid,"set_free",f"{state['target']}={count}")
    ...
    if action == "admin_user_add_searches":
        ...
        try:
            ...
            await msg.answer("?")
            return
        add_extra_searches(state["target"], count)
        log_action(uid,"add_searches",f"{state['target']}+{count}")
        await msg.answer(f"? +{count}")
        return
    ...
    if action == "admin_user_set_balance":
        ...
        try:
            ...
            await msg.answer("?")
            return
        set_balance(state["target"], amount)
        log_action(uid,"set_bal",f"{state['target']}={amount}")
        await msg.answer(f"? {amount:.1f}?")
        return
    ...
    if action == "admin_user_add_days":
        ...
        try:
            ...
            await msg.answer("?")
            return
        end = give_subscription(state["target"], days)
        log_action(uid,"add_days",f"{state['target']}+{days}")
    ...
    if action == "admin_user_add_vip_days":
        ...
        try:
            ...
            await msg.answer("?")
            return
        end = give_vip(state["target"], days)
        log_action(uid,"add_vip",f"{state['target']}+{days}")
    ...
    if action == "admin_user_msg":
        user_states.pop(uid,None)
        try:
            await bot.send_message(state["target"], msg.text.strip(), parse_mode="HTML")
            await msg.answer("?")
        except Exception as e:
    ...
    if action == "admin_create_promo_discount":
        ...
        if text_val.endswith("%"):
            try:
                percent = int(text_val[:-1]); assert 1 <= percent <= 99
            except:
                await msg.answer("? 1-99%")
                return
        else:
            try:
                stars_disc = int(text_val); assert stars_disc > 0
            except:
                await msg.answer("? Число > 0")
                return
        user_states[uid] = {"action":"admin_create_promo_uses","code":state["code"],"percent":percent,"stars":stars_disc}
        await msg.answer("?? Макс использований (число):")
        return

    if action == "admin_create_promo_uses":
        try:
            max_uses = int(msg.text.strip()); assert max_uses > 0
        except:
            await msg.answer("? Число > 0")
            return
        user_states.pop(uid, None)
        ok = create_promocode(state["code"], discount_percent=state["percent"], discount_stars=state["stars"], max_uses=max_uses, created_by=uid)
        if ok:
    ...
    if action == "admin_add_session_api_id":
        ...
        await msg.answer("<b>api_hash:</b>", parse_mode="HTML")
        return
    ...
    if action == "admin_add_session_api_hash":
        ...
        await msg.answer("<b>Телефон (с +):</b>", parse_mode="HTML")
        return
    ...
    if action == "admin_add_session_phone":
        ...
        try:
            ...
            pool.clients.append(cl)
            pool._init(idx)
            pool.word_reserved_idx = len(pool.clients) - 1
            ...
            try:
                ...
                with open("added_sessions.json", "w", encoding="utf-8") as f:
    ...
    if action == "admin_refs_check_input":
        ...
        if fraud["fraud"]:
            ...
            text += "? Ок\n\n"
        for r in refs[:10]:
            rn = f"@{r['uname']}" if r['uname'] else f"ID:{r['uid']}"
            text += f"• {rn} {r['created']}\n"
        kb = InlineKeyboardBuilder()
        kb.button(text="??", callback_data=f"au_back_{target}")
        kb.button(text="??", callback_data="a_refs")
        kb.adjust(1)
    ...
    if action == "ctl_set_value":
        ...
        if key.startswith("price_"):
            ...
            if pk in PRICES:
                try:
                    PRICES[pk]["stars"] = int(value)
                    config.setdefault("prices", {})[pk] = int(value)
        ...
        int_keys = [
            "free_searches","free_count","premium_count","vip_count",
            "premium_searches_limit","vip_searches_limit",
            "search_price_stars","ref_bonus","search_cooldown","min_withdraw",
            "farm_target_default","farm_target_beautiful","farm_target_meaningful",
            "farm_fragment_ttl_min"
        ]
        bool_keys = ["farm_enabled"]
        ...
        if key in int_keys:
            ...
            lv = value.lower()
            config[key] = lv in ("1","true","yes","on","да","вкл")
        else:
            config[key] = value

        save_bot_config(config)
        apply_config(config)
        log_action(uid,"config",f"{key}={value}")
        await msg.answer(f"? <code>{key}</code> = <code>{value}</code>", parse_mode="HTML")
        return
    ...
    if action == "ctl_set_text":
        ...
        save_bot_config(config)
        log_action(uid,"config_text",key)
...
async def cb_admin(cb: CallbackQuery):
    ...
    text = (
        ...
        f"?? Сессии: {sl}\n"
        f"?? Маркет: <code>{pending_m}</code> модерация | ?? <code>{disputes_m}</code> споров"
    )
    ...
    kb.button(text="?? Ферма", callback_data="adm_farm")
    kb.button(text="?? Рассылка", callback_data="adm_broadcast")
    kb.button(text="?? Настройки", callback_data="a_control")
...
@dp.callback_query(F.data == "adm_farm")
async def cb_adm_farm(cb: CallbackQuery):
    if cb.from_user.id not in ADMIN_IDS:
        return
    await answer_cb(cb)
    ...
    cfg = load_bot_config()
    targets = farm_get_targets()
    farm_on = "? ВКЛ" if cfg.get("farm_enabled", True) else "? ВЫКЛ"
    wr = st.get("word_reserved_idx")
    wr_text = f"#{wr+1}" if wr is not None else "—"

    text = (
        f"?? <b>Ферма юзов</b>\n\n"
        f"Статус: {farm_on}\n"
        f"Фарм-сессий: <code>{st['farm_sessions']}</code>\n"
        f"Сессия по слову: <code>{wr_text}</code> "
        f"({'??' if st['word_sessions'] > 0 else '??'})\n\n"
        f"?? Активных всего: <code>{st['active_total']}</code>\n"
        f"? Выдано: <code>{st['issued_total']}</code>\n"
        f"?? Умерло: <code>{st['dead_total']}</code>\n\n"
        f"?? Дефолт: <code>{st['counts'].get('default',0)}</code>/<code>{targets['default']}</code>\n"
        f"?? Красивые: <code>{st['counts'].get('beautiful',0)}</code>/<code>{targets['beautiful']}</code>\n"
        f"?? Словарные: <code>{st['counts'].get('meaningful',0)}</code>/<code>{targets['meaningful']}</code>\n"
    )

    kb = InlineKeyboardBuilder()
    kb.button(text="?? Обновить", callback_data="adm_farm")
    kb.button(text="?? Очистить", callback_data="adm_farm_clean")
    if cfg.get("farm_enabled", True):
        kb.button(text="? Выключить", callback_data="adm_farm_toggle")
    else:
        kb.button(text="?? Включить", callback_data="adm_farm_toggle")
    kb.button(text="?? Админ", callback_data="cmd_admin")
    kb.adjust(2)
...
@dp.callback_query(F.data == "adm_farm_toggle")
async def cb_adm_farm_toggle(cb: CallbackQuery):
    if cb.from_user.id not in ADMIN_IDS:
        return
    await answer_cb(cb)
    cfg = load_bot_config()
    cfg["farm_enabled"] = not cfg.get("farm_enabled", True)
    save_bot_config(cfg)
    log_action(cb.from_user.id, "farm_toggle", str(cfg["farm_enabled"]))
    await cb_adm_farm(cb)


@dp.callback_query(F.data == "adm_farm_clean")
async def cb_adm_farm_clean(cb: CallbackQuery):
    if cb.from_user.id not in ADMIN_IDS:
        return
    await answer_cb(cb)
    farm_cleanup_cache()
    log_action(cb.from_user.id, "farm_clean", "")
    await cb_adm_farm(cb)
...
async def cb_adm_sessions(cb: CallbackQuery):
    ...
    kb.button(text=f"?? {'Без сессий' if is_sessions_checker() else 'Сессии'}", callback_data="a_toggle_checker")
...
async def cb_au(cb: CallbackQuery):
    ...
    if action == "adds":
        ...
        remove_subscription(target); log_action(cb.from_user.id,"remove_sub",str(target)); await show_user_panel(cb, target)
    elif action == "addv":
        ...
        remove_vip(target); log_action(cb.from_user.id,"remove_vip",str(target)); await show_user_panel(cb, target)
    elif action == "resetd":
        conn = sqlite3.connect(DB); c = conn.cursor()
        c.execute("UPDATE users SET daily_searches_used=0 WHERE uid=?", (target,))
        conn.commit(); conn.close()
        log_action(cb.from_user.id,"reset_daily",str(target)); await show_user_panel(cb, target)
    elif action == "ban":
        ban_user(target); log_action(cb.from_user.id,"ban",str(target)); await show_user_panel(cb, target)
    elif action == "unban":
        unban_user(target); log_action(cb.from_user.id,"unban",str(target)); await show_user_panel(cb, target)
    elif action == "hist":
...
async def cb_asessions(cb: CallbackQuery):
    ...
    for i in range(len(pool.clients)):
        ...
        if st == 'dead':
            ...
            kb.button(text=f"?? #{i+1}", callback_data=f"a_kill_{i}")
    kb.button(text="? Добавить", callback_data="a_add_session")
    kb.button(text="? Reconnect all", callback_data="a_reconnect_all")
...
async def cb_kill(cb: CallbackQuery):
    ...
    if i < len(pool.clients):
        pool.status[i] = 'dead'
        pool.cooldown_until[i] = time.time() + 9999
        try:
            await pool.clients[i].disconnect()
        except:
            pass
...
async def cb_revive(cb: CallbackQuery):
    ...
    if i < len(pool.clients):
        ...
        log_action(cb.from_user.id,"revive",str(i))
    await cb_asessions(cb)
...
async def cb_akeys(cb: CallbackQuery):
    ...
    kb.button(text="?? Своё", callback_data="gk_custom")
    kb.button(text="??", callback_data="cmd_admin")
    kb.adjust(2)
...
async def cb_gk(cb: CallbackQuery):
    ...
    days = int(val)
    key = generate_key(days, f"D{days}")
    log_action(cb.from_user.id,"key",key)
    kb = InlineKeyboardBuilder()
...
async def cb_att(cb: CallbackQuery):
    ...
    if not tasks:
        kb = InlineKeyboardBuilder()
        kb.button(text="??", callback_data="cmd_admin")
        await edit_msg(cb.message, "?? Нет", kb.as_markup())
        return
...
async def cb_arefs_check(cb: CallbackQuery):
    if cb.from_user.id not in ADMIN_IDS:
        return
    await answer_cb(cb)
    user_states[cb.from_user.id] = {"action":"admin_refs_check_input"}
    kb = InlineKeyboardBuilder()
    kb.button(text="?", callback_data="a_refs")
    await edit_msg(cb.message, "?? <b>ID/@username:</b>", kb.as_markup())
...
async def cb_apromos(cb: CallbackQuery):
    ...
    if not promos:
        text += "<i>Нет</i>"
    kb.button(text="?", callback_data="a_addp")
    kb.button(text="??", callback_data="cmd_admin")
    kb.adjust(1)
    await edit_msg(cb.message, text, kb.as_markup())
...
async def cb_aendp(cb: CallbackQuery):
    if cb.from_user.id not in ADMIN_IDS:
        return
    await answer_cb(cb)
    end_promotion(int(cb.data[7:]))
    log_action(cb.from_user.id, "promo_end", cb.data[7:])
    await cb_apromos(cb)
...
async def cb_abl(cb: CallbackQuery):
    ...
    if not bl:
        text += "<i>Пусто</i>"
    kb.button(text="?", callback_data="bl_add")
    kb.button(text="??", callback_data="cmd_admin")
    kb.adjust(2,1)
    await edit_msg(cb.message, text, kb.as_markup())
...
async def cb_bl_del(cb: CallbackQuery):
    if cb.from_user.id not in ADMIN_IDS:
        return
    await answer_cb(cb)
    remove_blacklist(cb.data[7:])
    log_action(cb.from_user.id, "bl_del", cb.data[7:])
    await cb_abl(cb)
...
async def cb_araffle(cb: CallbackQuery):
    ...
    conn = sqlite3.connect(DB)
    c = conn.cursor()
    total = c.execute("SELECT COUNT(*) FROM users WHERE banned=0").fetchone()[0]
    conn.close()
    kb = InlineKeyboardBuilder()
    kb.button(text="?? 1д", callback_data="raf_1")
    kb.button(text="?? 3д", callback_data="raf_3")
    kb.button(text="?? 7д", callback_data="raf_7")
    kb.button(text="?? 30д", callback_data="raf_30")
    kb.button(text="??", callback_data="cmd_admin")
    kb.adjust(2,2,1)
...
async def cb_raf(cb: CallbackQuery):
    ...
    conn = sqlite3.connect(DB)
    c = conn.cursor()
    c.execute("SELECT uid,uname FROM users WHERE banned=0")
    au = c.fetchall()
    conn.close()
    if not au:
        return
    w = random.choice(au)
    end = give_subscription(w[0], days)
    name = f"@{w[1]}" if w[1] else f"ID:{w[0]}"
    log_action(cb.from_user.id, "raffle", f"{w[0]}+{days}d")
    await edit_msg(cb.message, f"?? <b>{name}</b>\n?? {days}дн до {end}")
    try:
        await bot.send_message(w[0], f"?? <b>{days}дн Premium!</b> До: {end}", parse_mode="HTML")
    except:
        pass
...
async def cb_alog(cb: CallbackQuery):
    ...
    if not logs:
        text += "<i>Пусто</i>"
    kb = InlineKeyboardBuilder()
    kb.button(text="??", callback_data="a_log")
    kb.button(text="??", callback_data="cmd_admin")
    kb.adjust(1)
    await edit_msg(cb.message, text, kb.as_markup())
...
async def cb_control(cb: CallbackQuery):
    ...
    text = (
        ...
        f"?? Ферма: {'ВКЛ' if config.get('farm_enabled',True) else 'ВЫКЛ'}\n"
...
async def cb_ctl_num(cb: CallbackQuery):
    ...
    kb.button(text=f"Ферма: {'ВКЛ' if config.get('farm_enabled',True) else 'ВЫКЛ'}", callback_data="setv_farm_enabled")
    kb.button(text=f"Фарм дефолт: {config.get('farm_target_default',60)}", callback_data="setv_farm_target_default")
    kb.button(text=f"Фарм красивые: {config.get('farm_target_beautiful',60)}", callback_data="setv_farm_target_beautiful")
    kb.button(text=f"Фарм словарные: {config.get('farm_target_meaningful',60)}", callback_data="setv_farm_target_meaningful")
    kb.button(text=f"TTL Fragment: {config.get('farm_fragment_ttl_min',15)}", callback_data="setv_farm_fragment_ttl_min")
    kb.button(text=f"Контакт: @{config['pay_contact']}", callback_data="setv_pay_contact")
    kb.button(text="??", callback_data="a_control")
    kb.adjust(1)
...
async def cb_ctl_texts(cb: CallbackQuery):
    ...
    def pv(k):
        v = config.get(k,"")
        return f"<code>{v[:25]}...</code>" if len(v) > 25 else (f"<code>{v}</code>" if v else "<i>—</i>")
...
async def cb_sett_reset(cb: CallbackQuery):
    ...
    await answer_cb(cb)
    config = load_bot_config()
    for k in ["text_welcome","text_found","text_empty","text_profile_header","text_shop_header"]:
        config[k] = ""
    save_bot_config(config)
    await cb_ctl_texts(cb)
...
async def cb_sett(cb: CallbackQuery):
    ...
    kb = InlineKeyboardBuilder()
    kb.button(text="?", callback_data="ctl_texts")
    await edit_msg(cb.message, "?? <b>Новый текст (HTML):</b>", kb.as_markup())
...
async def cb_ctl_modes(cb: CallbackQuery):
    ...
    for key,m in SEARCH_MODES.items():
        on = config.get(f"mode_{key}", True)
        prem = config.get(f"mode_{key}_premium", m.get("_default_premium", False))
        ei = "?" if on else "?"
        pi = "??" if prem else "??"
...
async def cb_ctl_prices(cb: CallbackQuery):
    ...
    kb = InlineKeyboardBuilder()
    for k,p in PRICES.items():
        text += f"• {p['label']} — <code>{p['stars']}?</code> ({p['rub']}?)\n"
        kb.button(text=f"?? {p['label']}", callback_data=f"setv_price_{k}")
    kb.button(text="??", callback_data="a_control")
    kb.adjust(2,1)
    await edit_msg(cb.message, text, kb.as_markup())
...
async def cb_ctl_ch(cb: CallbackQuery):
    ...
    if not chs:
        text += "<i>Нет</i>"
    kb.button(text="?", callback_data="ch_add")
    kb.button(text="??", callback_data="a_control")
    kb.adjust(1)
    await edit_msg(cb.message, text, kb.as_markup())
...
async def cb_ch_add(cb: CallbackQuery):
    if cb.from_user.id not in ADMIN_IDS:
        return
    await answer_cb(cb)
    user_states[cb.from_user.id] = {"action":"ctl_add_channel"}
    kb = InlineKeyboardBuilder()
    kb.button(text="?", callback_data="ctl_channels")
...
async def cb_ctl_reset(cb: CallbackQuery):
    ...
    await answer_cb(cb)
    kb = InlineKeyboardBuilder()
    kb.button(text="?? Да", callback_data="ctl_reset_yes")
    kb.button(text="?", callback_data="a_control")
    kb.adjust(1)
    await edit_msg(cb.message, "?? <b>Сбросить ВСЕ?</b>", kb.as_markup())
...
async def cb_ctl_reset_yes(cb: CallbackQuery):
    ...
    log_action(cb.from_user.id, "config_reset", "all")
    kb = InlineKeyboardBuilder()
    kb.button(text="??", callback_data="a_control")
...
async def cb_setv(cb: CallbackQuery):
    ...
    key = cb.data[5:]
    names = {
        "free_searches":"Free поисков","free_count":"Free юзов","premium_count":"Prem юзов",
        ...
        "pay_contact":"Контакт","ref_bonus":"Реф бонус","search_cooldown":"КД (сек)",
        "min_withdraw":"Мин вывод ?","farm_enabled":"Ферма ВКЛ/ВЫКЛ",
        "farm_target_default":"Фарм дефолт","farm_target_beautiful":"Фарм красивые",
        "farm_target_meaningful":"Фарм словарные","farm_fragment_ttl_min":"TTL Fragment"
    }
    if key.startswith("price_"):
        pk = key[6:]
        p = PRICES.get(pk,{})
        name = f"Цена {p.get('label',pk)} ?"
    else:
        name = names.get(key,key)

    config = load_bot_config()
    if key.startswith("price_"):
        current = PRICES.get(key[6:],{}).get("stars","?")
    elif key in config:
        current = config[key]
    else:
        current = "?"
    ...
    kb = InlineKeyboardBuilder()
    kb.button(text="?", callback_data="ctl_numbers")
...
async def cb_update(cb: CallbackQuery):
    if cb.from_user.id not in ADMIN_IDS:
        return
    await answer_cb(cb)
    await edit_msg(cb.message, "?? Скачиваю...")
    try:
        result = subprocess.run(
            ["curl","-s","https://raw.githubusercontent.com/yarmukhamedovsergey-creator/app.zip/main/bot.py","-o","/root/bot.py"],
...
async def cb_restart(cb: CallbackQuery):
    ...
    for aid in ADMIN_IDS:
        try:
            await bot.send_message(aid, "?? <b>Перезапуск...</b>", parse_mode="HTML")
        except:
            pass
...
async def cb_logs(cb: CallbackQuery):
    ...
    try:
        ...
        logs = "systemd не настроен"
    kb = InlineKeyboardBuilder()
    kb.button(text="??", callback_data="a_logs")
    kb.button(text="??", callback_data="a_logs_full")
    kb.button(text="??", callback_data="cmd_admin")
    kb.adjust(1)
...
async def cb_logs_full(cb: CallbackQuery):
    ...
    await answer_cb(cb)
    try:
        r = subprocess.run(["journalctl","-u","hunter","-n","500","--no-pager"], capture_output=True, text=True, timeout=15)
        c = r.stdout or "Пусто"
    except:
        c = "Ошибка"
...
async def cb_server(cb: CallbackQuery):
    ...
    text = f"?? <b>Сервер</b>\n\n? {up}\n? {load}\n\n??\n<pre>{mem}</pre>\n??\n<pre>{disk}</pre>"
    kb = InlineKeyboardBuilder()
    kb.button(text="??", callback_data="a_server")
    kb.button(text="??", callback_data="cmd_admin")
    kb.adjust(1)
    await edit_msg(cb.message, text, kb.as_markup())
...
async def cb_mmod_ok(cb: CallbackQuery):
    ...
    if lot:
        try:
            await bot.send_message(lot["seller_uid"], "? <b>Лот одобрен!</b>", parse_mode="HTML")
        except:
            pass
    await cb_a_mmod(cb)
...
async def cb_mmod_no(cb: CallbackQuery):
    ...
    if lot:
        try:
            await bot.send_message(lot["seller_uid"], "? <b>Лот отклонён</b>", parse_mode="HTML")
        except:
            pass
    await cb_a_mmod(cb)
...
async def cb_dwin_b(cb: CallbackQuery):
    ...
    if ok and charge_id and lot:
        try:
            await bot.refund_star_payment(lot["buyer_uid"], charge_id)
        except:
            add_balance(lot["buyer_uid"], lot["price"])
    elif ok and lot:
        add_balance(lot["buyer_uid"], lot["price"])
...
async def cb_create_promo(cb: CallbackQuery):
    ...
    kb = InlineKeyboardBuilder()
    kb.button(text="?", callback_data="a_promocodes")
    await edit_msg(cb.message,
        "?? <b>Создание промокода</b>\n\n"
        "Введите код (или <code>auto</code> для автогенерации):",
        kb.as_markup())
            ...
            for db in REMINDER_DAYS:
                for u in get_expiring_users(db):
                    ...
                    if rk in u.get("last_reminder",""):
                        continue
                    kb = InlineKeyboardBuilder()
                    kb.button(text="?? Продлить", callback_data="shop_premium")
                    try:
...
async def monitor_loop():
    ...
    while True:
        try:
            ...
            for mon in get_active_monitors():
                try:
                    status = await check_username(mon["username"])
                    if status in ("free","maybe_free"):
                        update_monitor_status(mon["id"],"free")
                        try:
                            kb = InlineKeyboardBuilder()
                            kb.button(text="??", callback_data=f"eval_{mon['username']}")
    ...
    if action == "admin_create_promo_code":
        ...
        await msg.answer(
            f"🏷 Код: <code>{code}</code>\n\n"
            f"Введите скидку:\n"
            f"• <code>10%</code> — процент\n"
            f"• <code>50</code> — фикс в ⭐",
            parse_mode="HTML"
        )
        return
    ...
    if action == "admin_create_promo_discount":
        ...
        user_states[uid] = {
            "action":"admin_create_promo_uses",
            "code":state["code"],
            "percent":percent,
            "stars":stars_disc
        }
    ...
    if action == "admin_create_promo_uses":
        ...
        ok = create_promocode(
            state["code"],
            discount_percent=state["percent"],
            discount_stars=state["stars"],
            max_uses=max_uses,
            created_by=uid
        )
        if ok:
    ...
    if action == "admin_add_session_phone":
        ...
        try:
            ...
            if pool.word_reserved_idx is None:
                pool.word_reserved_idx = idx
    ...
    if action == "ctl_set_value":
        ...
        int_keys = [
            "free_searches","free_count","premium_count","vip_count",
            "premium_searches_limit","vip_searches_limit","search_price_stars",
            "ref_bonus","search_cooldown","min_withdraw",
            "farm_target_default","farm_target_beautiful","farm_target_meaningful",
            "farm_fragment_ttl_min"
        ]
...
async def cb_adm_farm(cb: CallbackQuery):
    ...
    text = (
        ...
        f"📦 Активных всего: <code>{st['active_total']}</code>\n"
        f"✅ Выдано: <code>{st['issued_total']}</code>\n"
        f"☠️ Умерло: <code>{st['dead_total']}</code>\n\n"
        f"🎲 Дефолт: <code>{st['counts'].get('default',0)}</code>/<code>{targets['default']}</code>\n"
        f"💎 Красивые: <code>{st['counts'].get('beautiful',0)}</code>/<code>{targets['beautiful']}</code>\n"
        f"📖 Словарные: <code>{st['counts'].get('meaningful',0)}</code>/<code>{targets['meaningful']}</code>\n"
    )
    ...
    if cfg.get("farm_enabled", True):
        ...
        kb.button(text="▶️ Включить", callback_data="adm_farm_toggle")
    kb.button(text="🔙 Админ", callback_data="cmd_admin")
    kb.adjust(2)
...
async def cb_au(cb: CallbackQuery):
    ...
    if action == "adds":
        ...
        log_action(cb.from_user.id, "remove_sub", str(target))
        await show_user_panel(cb, target)
    elif action == "addv":
        user_states[cb.from_user.id] = {"action":"admin_user_add_vip_days","target":target}
        kb = InlineKeyboardBuilder(); kb.button(text="❌", callback_data=f"au_back_{target}")
        ...
        remove_vip(target)
        log_action(cb.from_user.id, "remove_vip", str(target))
        await show_user_panel(cb, target)
    elif action == "resetd":
        conn = sqlite3.connect(DB)
        c = conn.cursor()
        ...
        ban_user(target)
        log_action(cb.from_user.id, "ban", str(target))
        await show_user_panel(cb, target)
    elif action == "unban":
        unban_user(target)
        log_action(cb.from_user.id, "unban", str(target))
        await show_user_panel(cb, target)
    elif action == "hist":
        hist = get_history(target, 15)
        text = f"📜 <b>{target}</b>\n\n"
...
async def cb_akeys(cb: CallbackQuery):
    ...
    for k, p in PRICES.items():
        kb.button(text=p["label"], callback_data=f"gk_{p['days']}")
    kb.button(text="✏️ Своё", callback_data="gk_custom")
    kb.button(text="🔙", callback_data="cmd_admin")
    kb.adjust(2)
    await edit_msg(cb.message, "🔑 <b>Срок:</b>", kb.as_markup())
...
async def cb_gk(cb: CallbackQuery):
    ...
    if val == "custom":
        ...
        await edit_msg(cb.message, "✏️ <b>Дней:</b>", kb.as_markup())
        return
...
async def cb_ctl_num(cb: CallbackQuery):
    ...
    kb.button(text=f"Вывод: {config['min_withdraw']}⭐", callback_data="setv_min_withdraw")
    kb.button(text=f"Контакт: @{config['pay_contact']}", callback_data="setv_pay_contact")
    kb.button(text=f"Farm def: {config.get('farm_target_default',60)}", callback_data="setv_farm_target_default")
    kb.button(text=f"Farm beauty: {config.get('farm_target_beautiful',60)}", callback_data="setv_farm_target_beautiful")
    kb.button(text=f"Farm dict: {config.get('farm_target_meaningful',60)}", callback_data="setv_farm_target_meaningful")
    kb.button(text=f"Farm TTL frag: {config.get('farm_fragment_ttl_min',15)}", callback_data="setv_farm_fragment_ttl_min")
    kb.button(text="🔙", callback_data="a_control")
    kb.adjust(1)
    await edit_msg(cb.message, "🔢 <b>Нажмите чтобы изменить</b>", kb.as_markup())
...
async def cb_ctl_btns(cb: CallbackQuery):
    ...
    config = load_bot_config()
    btns = {
        "tiktok":"🎁 TikTok","roulette":"🎰 Рулетка","monitor":"👁 Мониторинг",
        "shop":"🏪 Магазин","support":"🤖 Донат","referral":"👥 Рефералы"
    }
    text = "🔘 <b>Кнопки</b>\n\n"
    kb = InlineKeyboardBuilder()
    for k, n in btns.items():
...
async def cb_ctl_reset(cb: CallbackQuery):
    ...
    await answer_cb(cb)
    kb = InlineKeyboardBuilder()
    kb.button(text="⚠️ Да", callback_data="ctl_reset_yes")
    kb.button(text="❌", callback_data="a_control")
    kb.adjust(1)
...
async def cb_setv(cb: CallbackQuery):
    ...
    key = cb.data[5:]
    names = {
        "free_searches":"Free поисков",
        "free_count":"Free юзов",
        "premium_count":"Prem юзов",
        "vip_count":"VIP юзов",
        "premium_searches_limit":"Prem поисков/день",
        "vip_searches_limit":"VIP поисков/день",
        "search_price_stars":"Цена/поиск ⭐",
        "pay_contact":"Контакт",
        "ref_bonus":"Реф бонус",
        "search_cooldown":"КД (сек)",
        "min_withdraw":"Мин вывод ⭐",
        "farm_target_default":"Ферма дефолт",
        "farm_target_beautiful":"Ферма красивые",
        "farm_target_meaningful":"Ферма словарные",
        "farm_fragment_ttl_min":"TTL Fragment (мин)",
    }
...
async def cb_exconfirm(cb: CallbackQuery):
    ...
    if completed and ex_data:
        ...
        await edit_msg(cb.message,
            ...
            f"🔗 <a href='https://t.me/{you_get}'>Открыть в Telegram</a>\n"
            f"💎 <a href='https://fragment.com/username/{you_get}'>Проверить на Fragment</a>\n\n"
            f"⚠️ Не забудь — юзернейм нужно <b>успеть забрать</b> до того как его займут!"
        )
...
# ═══════════════════════ CALLBACKS: ПРОФИЛЬ / РУЛЕТКА / РЕФЫ / ТИКТОК ═══════════════════════
...
async def cb_profile(cb: CallbackQuery):
    ...
    kb = InlineKeyboardBuilder()
    if ar_on:
        kb.button(text="🔄 Выкл авто", callback_data="toggle_renew")
    else:
        kb.button(text="🔄 Вкл авто", callback_data="toggle_renew")
    kb.button(text="📜 История", callback_data="util_hist")
    kb.button(text="🔑 Ключ", callback_data="cmd_activate")
    kb.button(text="🎁 Подарить", callback_data="gift_prem")
    if bal >= MIN_WITHDRAW:
...
async def cb_tt(cb: CallbackQuery):
    ...
    await edit_msg(cb.message,
        f"🎁 <b>TikTok</b>\n\n"
        f"1️⃣ Найди видео\n"
        f"2️⃣ {TIKTOK_SCREENSHOTS_NEEDED} комментов\n"
        f"💬 <code>{TIKTOK_COMMENT_TEXT}</code>\n"
        f"3️⃣ Скрины\n"
        f"4️⃣ 🎁 {TIKTOK_REWARD_GIFT}",
        kb.as_markup()
    )
...
def setup_systemd():
    ...
    try:
        with open(sp, "w", encoding="utf-8") as f:
            f.write(
                f"[Unit]\n"
                f"Description=Hunter Bot\n"
                f"After=network.target\n\n"
                f"[Service]\n"
                f"Type=simple\n"
                f"User=root\n"
                f"WorkingDirectory={bd}\n"
                f"ExecStart={pp} {bp}\n"
                f"Restart=always\n"
                f"RestartSec=3\n"
...
async def main():
    ...
    logger.info("━" * 30)
    logger.info(f"🚀 @{bot_info.username} v26.0")
    ...
    for i in range(4):
        asyncio.create_task(farm_worker(i + 1))
