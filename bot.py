...
def get_ref_top_by_period(start_date, limit=10):
    ...
    for r in rows:
        u = get_user(r[0])
        result.append({"uid": r[0], "uname": u.get("uname", ""), "ref_count": r[1]})
    return result
...
async def check_referral_subscription(referrer_uid):
    ...
    for ref in refs:
        ref_uid = ref["uid"]
        not_subscribed = await check_subscribed(ref_uid)
        if not_subscribed:
            remove_referral(referrer_uid, ref_uid)
            removed += 1
    return removed
    ...
    if action == "gift_username":
        ...
        if method == "stars":
            await bot.send_invoice(uid, title="🎁 Подарок " + p["label"] + " для @" + tu,
                description="Premium " + p["label"] + " для @" + tu, payload="gift_" + plan + "_" + str(tuid) + "_" + str(uid),
...
async def succ_pay(msg: Message):
    ...
    if parts[0] == "sub" and len(parts) >= 3:
        ...
        item = c.fetchone()
        if item:
            seller_uid, username, price = item
            sa = round(price * 0.9, 1)
            now = datetime.now().strftime("%Y-%m-%d %H:%M")
            c.execute("UPDATE market_listings SET status='escrow', sold_to=?, sold_at=? WHERE id=?", (buyer_uid, now, lid))
            c.execute("INSERT INTO market_transactions (listing_id,buyer_uid,seller_uid,username,price,method,status,created) VALUES (?,?,?,?,?,?,?,?)",
                      (lid, buyer_uid, seller_uid, username, price, "tg_stars", "escrow", now))
...
async def cb_arefs(cb: CallbackQuery):
    ...
    kb.button(text="🔍 Проверить юзера", callback_data="a_refs_check")
...
async def cmd_webapp(msg: Message):
    ...
    ensure_user(uid, msg.from_user.username)
    kb = InlineKeyboardBuilder()
    kb.button(text="🏪 Маркет", web_app=WebAppInfo(url=f"{WEBAPP_URL}/webapp"))
    kb.button(text="🎮 Игры", web_app=WebAppInfo(url=f"{WEBAPP_URL}/webapp/games"))
    kb.button(text="💰 Продать", web_app=WebAppInfo(url=f"{WEBAPP_URL}/webapp/sell"))
